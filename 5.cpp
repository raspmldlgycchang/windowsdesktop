		...
		*/
		...
		for (unsigned int j = 0; j < m_max_user_cnt; j++) {
			for (i = 0; i < m_max_count; i++) {
				...
				p_handle++;//주석3참고_방법1 한명의 user에서 문자열총개수m_max_count만큼 돌기위해 1차원구조체변수 원소하나 메모리크기만큼 증가
				cout << "동적메모리 해제 및 객체 소멸 완료" << endl;
			}
			delete[] mp_users[j];
			cout << "핸들객체관리자 mp_users[" << j << "]" << "동적메모리해제 및 객체소멸완료!" << endl;
...
int main(void)
{
	...
	HANDLE h_str_user1_1 = str_list_user1.SetString("땨량해");
	cout << h_str_user1_1 << ": HANDLE h_str_user1_1" << endl;

	HANDLE h_str_user1_2 = str_list_user1.SetString("도옹도옹니이~");
	cout << h_str_user1_2 << ": HANDLE h_str_user1_2" << endl;

	HANDLE h_str_user1_3 = str_list_user1.SetString("~아이잉~");
	cout << h_str_user1_3 << ": HANDLE h_str_user1_3" << endl;

	HANDLE h_str_user1_4 = str_list_user1.SetString("도옹도옹니이잉~");
	cout << h_str_user1_4 << ": HANDLE h_str_user1_4" << endl;

	HANDLE h_str_user1_5 = str_list_user1.SetString("땨량해까꿍이가");
	cout << h_str_user1_5 << ": HANDLE h_str_user1_5" << endl;

	HANDLE h_str_user1_6 = str_list_user1.SetString("오오오옴총총총");
	cout << h_str_user1_6 << ": HANDLE h_str_user1_6" << endl;

	HANDLE h_str_user1_7 = str_list_user1.SetString("까꿍땨량해");
